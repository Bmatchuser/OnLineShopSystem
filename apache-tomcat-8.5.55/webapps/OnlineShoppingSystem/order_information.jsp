...
<head>
    ...
    <script type="text/javascript">
		...
		if (confirm("你要确认发货吗！")) {
			location.href = "${pageContext.request.contextPath}/servlet/SendGoodsServlet?id="+ id;
			}
		}
...
<body>
	...
	<div class="col-lg-10" style="height: 600px;background-color: white">	
		...
		<div style="float: left">
		    <form action="${pageContext.request.contextPath}/servlet/OrderListServlet" class="form-inline" method="post">
		        <div class="form-group">
		            <label for="exampleInputName2">姓名</label>
		            <input name="username" style="width: 150px;" type="text" class="form-control" id="exampleInputName2" value="${condition.username[0]}">
		        </div>
		        <div class="form-group">
		            <label for="exampleInputEmail2">电话号码</label>
		            <input name="tel" style="width: 150px;" type="text" class="form-control" id="exampleInputEmail2" value="${condition.tel[0]}">
		        </div>
		        <div class="form-group">
		            <label for="exampleInputName2">地址</label>
		            <input name="address" style="width: 150px;" type="text" class="form-control" id="exampleInputName2" value="${condition.address[0]}" >
		        </div>
		        <button type="submit" class="btn btn-default">查询</button>
		    </form>
		</div>
		<div style="float: right">
		    <a href="${pageContext.request.contextPath}/add_order.jsp" class="btn btn-primary">添加订单</a>
		        ...
		        <c:forEach items="${pb.pageList}" var="order" varStatus="s">
		            <tr>
						...
						<c:if test="${order.status == 1}">
								<a  class="btn btn-danger"  href="javascript:sendGoods(${order.id});">发货</a>
						</c:if>
		                ...
		                <td>
		                    <a class="btn btn-info" href="${pageContext.request.contextPath}/servlet/FindOrderServlet?id=${order.id}">修改</a>
		                    <a class="btn btn-danger" href="javascript:deleteUser(${order.id});">删除</a>
		                </td>
		            </tr>
		        </c:forEach>
		    </table>
    ...
    <ul class="pagination" >
        ...
        <li>
            ...
            <a href="${pageContext.request.contextPath}/servlet/OrderListServlet?currentPage=${pb.currentPage-1}&pageRows=5&name=${condition.name[0]}&tel=${condition.tel[0]}&email=${condition.email[0]}" aria-label="Previous">
        ...
        <c:forEach begin="1" end="${pb.totalPage}" var="i">
            ...
            <c:if test="${pb.currentPage==i}">
                <li class="active">
                    <a href="${pageContext.request.contextPath}/servlet/OrderListServlet?currentPage=${i}&pageRows=5&name=${condition.name[0]}&tel=${condition.tel[0]}&email=${condition.email[0]}">
                            ${i}
                    </a>
                </li>
            </c:if>
        ...
        <li>
            ...
            <a href="${pageContext.request.contextPath}/servlet/OrderListServlet?currentPage=${pb.currentPage+1}&pageRows=5&name=${condition.name[0]}&tel=${condition.tel[0]}&email=${condition.email[0]}" aria-label="Next">
